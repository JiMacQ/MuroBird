...
if not exist assets\offline\offline_db.json (
  ...
  echo   python tools\fetch_media_v4.py --excel Aves_Murocomba_Completo_21-09-25.xlsx --out-db assets\offline\offline_db.json
  pause
  exit /b 1
)
if not exist tools\fetch_media_v4.py (
  echo ERROR: No se encuentra tools\fetch_media_v4.py
  pause
  exit /b 1
)
...
python tools\fetch_media_v4.py ^
